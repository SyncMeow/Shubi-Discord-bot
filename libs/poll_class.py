...
class PollActivity():
    ...
    def info_embed(self) -> discord.Embed:
        ...
        infoEmbed = discord.Embed(title = f"投票詳細資訊", description=descr_text(opts_n, users_n), 
                                  color= discord.Color.light_gray(), timestamp=datetime.now())
...
class PollCreateModal(discord.ui.Modal):
    ...
    async def generate_id(self, title:str, opts:list, author:discord.Member):
        global poll_activity
        while (True):
            id = ""
            for i in range(3):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if (len(options) <= 1):return await  interaction.followup.send("必須輸入至少兩個選項",ephemeral=True)
        
        id = await self.generate_id(self.SubjectContext, options, self.author)
